...
export default connect(toStudyProps)(class Study extends Component {
  constructor(props) {
    super(props);
    this.state = {
      active: 0,
  ...
  onSpace = (value) => {
    ...
    const isCorrect = _.includes(block.answers, value.replace(" ","").toLowerCase());
    if( isCorrect )
      this.onAnswer(value);
    return isCorrect;
  }
  ...
  onContinue = () => {
    ...
    if( cards.length === 1 ) {
      ...
      let i=0;
      while (i < nextCard.chinese.length && (_.isString(nextCard.chinese[i]) || !nextCard.chinese[i].isGap)) i++;
  ...
  getActive = () => {
    const {cards} = this.props;
    const valid = i => i<cards[0].chinese.length && _.isPlainObject(cards[0].chinese[i]) && cards[0].chinese[i].isGap;
    if( _.isArray(cards) && cards.length > 0 && !valid(this.state.active) ) {
        ...
        <Grid relaxed>
          ...
          <Grid.Row columns={3} style={controlRow}>
            <Grid.Column>
              ...
              <Button circular disabled={!ready} icon="external" target="_blank"
                      as={'a'} href={"https://tatoeba.org/eng/sentences/show/"+sid}/>
              <Button circular disabled={!done} icon="play"/>
            </Grid.Column>
            <Grid.Column textAlign="center">
              <Button.Group toggle>
                <Button onClick={this.onSwitchStyle('study')} active={style==='study'}>Study</Button>
                <Button.Or/>
                <Button onClick={this.onSwitchStyle('review')} active={style==='review'}>Review</Button>
              </Button.Group>
            </Grid.Column>
            <Grid.Column textAlign="right">
              { done
                ? <Button ref={this.handleContinueRef} onClick={this.onContinue} positive>Continue</Button>
                : <Button disabled={!ready} color="yellow">Show hint</Button> }
            </Grid.Column>
          </Grid.Row>
        </Grid>
      </div>
