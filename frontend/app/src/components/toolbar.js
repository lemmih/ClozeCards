...
const mkInline = (activeStyles, setEditorState) => class InlineStyle extends Component {
  toggle = (event) => {
    event.preventDefault();
    setEditorState(state => RichUtils.toggleInlineStyle(state, this.props.type));
  }
  render = () => {
    const active = activeStyles.has(this.props.type);
    return (
      <Button toggle onClick={this.toggle} active={active} icon={this.props.icon}>
        {this.props.children}
      </Button>);
  }
};

const mkBlock = (block, setEditorState) => class InlineStyle extends Component {
  toggle = (event) => {
    event.preventDefault();
    setEditorState(state => RichUtils.toggleBlockType(state, this.props.blockType));
  }
  render = () => {
